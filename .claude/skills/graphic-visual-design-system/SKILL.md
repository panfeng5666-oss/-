...
- **版式向古籍标本档案靠，不向海报靠。** 参照博物志、图鉴、标本卡的排布逻辑：词条、编号、注释、图注。
- 严格遵守纲领的**审美红线**：禁年画风与大红大绿、禁庙会风、禁神怪卡通化萌化、禁迷信功利许诺、禁导游词腔、禁「丰饶东海」调门、禁空泛形容词。
- **智性距离**是视觉口径 —— 冷峻、克制、有解剖感。土的事实不美化也不删除，靠版式的冷静承载它。
- 色彩工程须纳入有出处的本地色（如县志所载渔民衣**拷汁染 · 深赭**），不凭空定「海洋蓝」。
...
见 `knowledge/03-平面与视觉设计.md`（40 份）。版式与栅格、文字与字阶、色彩工程、日系极简美学均有对应专著；原研哉《设计中的设计》与知识库 05 的 MUJI 话语体系同源，视觉与文案应共用一套语汇。

色彩管理依据：Bruce Fraser《Real World Color Management》与《印刷色彩学》（色空间、ICC 特性文件、屏幕与印刷转换、专色与四色关系）。

⚠️ **三点注意**：
- 包装刀版结构取 `knowledge/06`（孙诚《包装结构设计》、潘松年《包装工艺学》）。
- **ΔE 允收阈值仍无标准出处** —— 色彩管理的方法与流程已有依据，但具体容差数值须以 `ISO 12647`（尤其 12647-2）或印厂作业规范为准。本文与案例中的 ΔE 数值属经验值，付印前向印厂索取《打样确认单》或《色彩管理作业规范》坐实。
- **Pantone 专色必须有实体色卡**（Formula Guide / TCX 棉布版）—— 屏幕呈色与油墨呈色不同，纯数字流程无法管控专色。

## 参考样例

- `samples/02-测试-海盐冷萃咖啡豆与粗陶随行杯礼盒.md`（模块三）
